...
def train_session_model_incremental(dataset_path: str = "data/processed/session_start_train.parquet",
    ...
    if is_first_time:
        ...
        # Extract dates from ingestion filenames
        ...
        for file_path in new_ingestion_files:
            ...
            try:
                ...
                ingestion_dates.append(dt.date())
            except ValueError:
                print(f"Warning: Could not parse date from {filename}")
        
        if not ingestion_dates:
            print("Could not extract dates from ingestion files. Using dataset filtering fallback.")
            ...
            if "date" in df.columns:
                ...
                if last_processed_file:
                    try:
                        ...
                        new_df = df[df["date"].dt.date > last_date].copy()
                    except:
            ...
            # Filter dataset to samples from dates in new ingestion files
            if "date" in df.columns:
                df["date"] = pd.to_datetime(df["date"])
                min_date = min(ingestion_dates)
                max_date = max(ingestion_dates) + timedelta(days=1)  # Add buffer
                new_df = df[
                    (df["date"].dt.date >= min_date) &
                    (df["date"].dt.date <= max_date)
                ].copy()
                print(f"Found {len(new_df)} samples from new ingestion files (dates: {min_date} to {max_date})")
            else:
                # Fallback: use last few rows
                new_df = df.tail(50).copy()
                print("No date column found: using last samples as fallback")
...
if __name__ == "__main__":
    train_session_model_incremental()
